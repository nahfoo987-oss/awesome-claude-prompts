    ...
    <section id="gallery" className="py-24 px-6 bg-white">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-plum/50 mt-3 text-sm max-w-sm mx-auto">Follow along for daily bakes, seasonal drops, and behind-the-scenes sweetness.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {posts.map((post) => (
            <div key={post.id} className="rounded-2xl overflow-hidden border border-blush">
              <iframe src={`https://www.instagram.com/${post.type}/${post.id}/embed/`} width="100%" height="480" frameBorder="0" scrolling="no" allowTransparency loading="lazy" className="w-full" />
            </div>
          ))}
        </div>
        <div className="text-center mt-10">
          <a href="https://instagram.com/jennysugarshack" target="_blank" rel="noopener noreferrer" className="btn-outline">Follow @jennysugarshack</a>
        </div>
      </div>
    </section>
  );
