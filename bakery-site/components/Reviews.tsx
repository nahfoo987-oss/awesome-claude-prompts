...
function PlatformBadge({ platform }: { platform: string }) {
  if (platform === "yelp") return (
    <span className="inline-flex items-center gap-1">
      <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="#d32323">
      ...
      <span className="text-[#d32323] font-bold text-xs">yelp</span>
    </span>
  );

  if (platform === "instagram") return (
    <span className="text-xs font-medium" style={{ background: "linear-gradient(45deg,#f09433,#bc1888)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
      Instagram
    </span>
  );
...
export default async function Reviews() {
  ...
  if (!reviews?.length) {
    return (
      <section className="py-24 px-6 bg-petal text-center">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {yelpAndOthers.map((r: any) => (
                <div key={r._id} className="bg-white rounded-2xl p-6 border border-blush">
                  ...
                  <p className="text-plum/70 leading-relaxed my-3 italic font-serif text-sm">
                    &ldquo;{r.text}&rdquo;
                  </p>
