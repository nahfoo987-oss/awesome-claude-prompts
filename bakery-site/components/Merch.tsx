    ...
    <section id="merch" className="py-24 bg-white">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-plum/50 mt-3 max-w-md mx-auto text-sm leading-relaxed">Branded goods for fellow baking lovers. Available for pickup with any order or shipped directly.</p>
        </div>
          ...
          {items.map((item: any) => (
            <div key={item._id} className="flex flex-col group">
              <div className="relative aspect-square rounded-2xl overflow-hidden bg-blush mb-4">
                <Image src={urlFor(item.image).width(400).url()} alt={item.name} fill className="object-cover transition-transform duration-300 group-hover:scale-105" />
              </div>
              <h3 className="font-serif text-plum text-base mb-1">{item.name}</h3>
              {item.description && <p className="text-xs text-plum/50 leading-relaxed mb-2">{item.description}</p>}
              <p className="text-sm font-medium text-pink-600 mt-auto">{item.price}</p>
            </div>
          ))}
        </div>
        <div className="text-center mt-12"><Link href="/order" className="btn-outline">Order merch with your next cake</Link></div>
      </div>
    </section>
  );
}
